...
def project_entities(nodes: tuple[GraphNode, ...]):
    ...
    documents = tuple(
        CanonicalDocument(
            id=node.id,
            document_id=project_fact(node.attributes["document_id"], node.provenance, f"graph.nodes[{node.id}].attributes.document_id"),
            document_type=project_fact(node.attributes["document_type"], node.provenance, f"graph.nodes[{node.id}].attributes.document_type") if node.attributes.get("document_type") else None,
            attributes={
                key: project_fact(value, node.provenance, f"graph.nodes[{node.id}].attributes.{key}")
                for key, value in node.attributes.items()
                if key not in {"document_id", "document_type"}
            },
        )
        for node in nodes if node.node_type == GraphNodeType.DOCUMENT
    )
    return {
